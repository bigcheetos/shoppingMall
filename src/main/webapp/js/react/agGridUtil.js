...
/**
 * DateTime Picker 사용법 
 * 1. 칼럼 정의 시 editable : true, cellEditor : 'dateTimePicker' 추가 
 * 2. 칼럼옵션에 컴포넌트 추가 
 * gridOpt.components = {
 *  dateTimePicker: getDateTimePicker('yy-mm-dd') // date format 지정시 포맷 전달 
 *  dateTimePicker: getDateTimePicker()  // 기본은 yyyy-mm-dd hh:MM:ss 로 됨 
 * };
 */
function getDateTimePicker(paramFmt) {
	var _this = this;
	_this.fmt = "yyyy-mm-dd hh:MM:ss";
	if(paramFmt){
		_this.fmt = paramFmt;
	}
    // function to act as a class
...
var ScrollGrid = {
		...
		,numberOnly : function(param){ 
			...
			return ScrollGrid.formatCurrency(param);
		}
		,formatCurrency : function (param){
			if(!param.value && param.value != "0") {
				return this.defaultBlank;
